...
function Node({ node, style, dragHandle, renderSuffix = (node) => false }: NodeRendererProps<Data> & { renderSuffix: (node: NodeApi<Data>) => React.ReactNode }) {
  ...
  return (
    <div
      ref={dragHandle}
      className={cn((node.level === 0) && "font-bold", !node.isInternal && node.isSelected && "rounded bg-brand/10 text-brand", "h-full flex gap-2 items-center justify-between cursor-pointer hover:bg-brand/5 hover:text-brand px-2 mx-2 transition-colors")}
      style={style}
