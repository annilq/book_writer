...
import { AssistantTextRender } from "@/components/Chat/chat-log";
...
export const AssistantText = ({ messages, message, data, onRefresh, action }: {
  messages: UIMessage[];
  message: UIMessage;
  data: string,
    ...
    <>
      {parts.map((part, i) => (
        <div key={part.content}>
          {part.type === "text" ? (
            <AssistantTextRender onRefresh={onRefresh} data={part.content} action={action} />
