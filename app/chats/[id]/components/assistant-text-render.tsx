...
const VersionCard = ({ messages, message, isActive = false, onMessageClick }: {
  ...
  return (
    <div className="my-4">
      <button
        className={`${isActive ? "bg-background" : " hover:border-muted-foreground/40 hover:bg-secondary hover:text-secondary-foreground"} inline-flex w-full items-center gap-2 rounded border-4 border-border p-1.5`}
...
const StreamingCard = ({ messages, message }: {
  ...
  return (
    <div className="my-4">
      <button
        disabled
        className="inline-flex w-full animate-pulse items-center gap-2 rounded border-4 border-border p-1.5"
      >
